...
function preload() {
  bombImg = loadImage("Images/bomb.png"); // Make sure the path is correct
}

function setup() {
  // Create a canvas and assign it an ID
  let canvas = createCanvas(windowHeight * 0.43, windowHeight * 0.93);
  canvas.id("bomb");

  // Parent the canvas to the specific div
  canvas.parent("bomb-holder");

  // Start capturing video from the webcam
  capture = createCapture(VIDEO);
  capture.size(940, 720);
  // Hide the default capture element as we'll draw the capture on the canvas
  capture.hide();
...
function draw() {
  // Display the webcam capture on the canvas
  let canvasAspectRatio = ((windowHeight * 0.43) / windowHeight) * 0.93;
  ...
  // Display the webcam capture on the canvas
  // Using offsetX and offsetY to center the video on the canvas
  image(capture, offsetX, offsetY, drawWidth, drawHeight);

  // Randomly add bombs
  if (random(1) < 0.005) {
    // Adjust the probability to control frequency
    bombs.push(new Bomb());
  }
  ...
  for (let i = bombs.length - 1; i >= 0; i--) {
    ...
    bombs[i].display();
    if (bombs[i].y > height) {
      bombs.splice(i, 1); // Remove bombs that have fallen off the screen
    }
  }
...
class Bomb {
  constructor() {
    // Choose from either 1/4, 1/2, or 3/4 the way across the canvas
    const positions = [0.3, 0.55];
    const randomIndex = floor(random(positions.length));
    this.x = width * positions[randomIndex];
    this.y = 0;
    this.speed = random(2, 5); // Adjust speed as needed
    this.size = 160;
  }
